...
- SHAP + LIME explainability per dimension
- API status bar showing model health

**Backend:** `artifacts/api-server` provides:
- `POST /api/classify` — keyword-based XAI classifier (mimics trained sklearn model output format)
- `GET /api/health` — model health/status endpoint
- `GET /api/healthz` — server health check

**Data storage:** Journal entries stored in localStorage (client-side, no DB required)
...
- Hooks: `src/hooks/use-journal.ts` — localStorage journal management
...
Database layer using Drizzle ORM with PostgreSQL. Currently no tables (journal entries use localStorage).
...
Generated React Query hooks: `useHealthCheck`, `useModelHealth`, `useClassifyDream`.
